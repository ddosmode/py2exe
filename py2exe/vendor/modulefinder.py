...
"""Переопределение importlib модуляimp.find_module (для наших целей)."""
    ...
    ### BEGIN CODE MODIFIED IN VENDOR ###
    if spec.loader is None and hasattr(spec, "submodule_search_locations"): # Implicit namespace packages
        return None, spec.submodule_search_locations._path[0], ("", "", _PY_NAMESPACE)
    ### END CODE MODIFIED IN VENDOR ###
...
class ModuleFinder:
    ...
    def load_package(self, fqname, pathname):
        ...
        if newname:
            fqname = newname
        ### BEGIN CODE MODIFIED IN VENDOR ###
        m = self.add_module(fqname, file=pathname, path=[pathname])
        #m.__file__ = pathname
        #m.__path__ = [pathname]
        ### END CODE MODIFIED IN VENDOR ###
